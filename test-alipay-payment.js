...
// 测试支付宝支付流程
function testAlipayPaymentFlow() {
  console.log('=== 支付宝支付流程测试 ===\n');
  ...
  try {
    ...
    // 4. 检查支付网关
    console.log('\n4. 检查支付网关...');
    const paymentGatewayPath = path.join(__dirname, 'src', 'services', 'payment', 'payment-gateway.ts');
    if (fs.existsSync(paymentGatewayPath)) {
      const gatewayContent = fs.readFileSync(paymentGatewayPath, 'utf8');
      if (gatewayContent.includes('alipayService')) {
        console.log('   ✅ 支付网关已集成支付宝服务');
      } else {
        console.log('   ❌ 支付网关未集成支付宝服务');
      }
    } else {
      console.log('   ❌ 支付网关文件不存在');
    }
    
    // 5. 检查支付路由
    console.log('\n5. 检查支付路由...');
    const paymentRoutePath = path.join(__dirname, 'src', 'routes', 'payment.ts');
    if (fs.existsSync(paymentRoutePath)) {
      const routeContent = fs.readFileSync(paymentRoutePath, 'utf8');
      if (routeContent.includes('/api/payment/alipay/notify')) {
        console.log('   ✅ 支付宝回调路由已配置');
      } else {
        console.log('   ❌ 支付宝回调路由未配置');
      }
    } else {
      console.log('   ❌ 支付路由文件不存在');
    }
    ...
    console.log('4. 需要重新部署 Railway 项目');
    console.log('5. 需要等待用户提供 WECHAT_PLATFORM_CERT');
